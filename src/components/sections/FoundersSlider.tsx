...
// ✅ PERBAIKAN 1: Path import disesuaikan (biasanya @ sudah mengarah ke src)
...
// ✅ PERBAIKAN 2: Pindahkan FounderCard ke ATAS sebelum dipanggil
const FounderCard = ({ founder, isMobile }: { founder: IFounder, isMobile?: boolean }) => (
  <div className={`bg-slate-900 border border-slate-800 rounded-3xl p-8 text-center h-full hover:border-blue-500/30 transition-all group relative overflow-hidden ${isMobile ? 'border-none bg-transparent shadow-none' : ''}`}>
      ...
      <div className="w-32 h-32 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-full mx-auto mb-6 flex items-center justify-center text-4xl text-white font-bold ring-4 ring-slate-800 group-hover:ring-blue-500/50 transition-all shadow-[0_0_20px_rgba(37,99,235,0.3)] relative z-10">
          {/* Jika founder.image adalah inisial (string), ini akan merender teks */}
          {founder.image}
      </div>
      ...
      <div className="flex justify-center gap-4 mt-auto">
         {founder.socials.linkedin && (
           <a href={founder.socials.linkedin} target="_blank" rel="noreferrer" className="p-2 rounded-full bg-slate-800 text-slate-400 hover:bg-blue-600 hover:text-white transition-all">
              <Linkedin size={18} />
           </a>
         )}
         {founder.socials.instagram && (
           <a href={founder.socials.instagram} target="_blank" rel="noreferrer" className="p-2 rounded-full bg-slate-800 text-slate-400 hover:bg-pink-600 hover:text-white transition-all">
              <Instagram size={18} />
           </a>
         )}
      </div>
...
const FoundersSlider: React.FC<FoundersSliderProps> = ({ darkBg = true }) => {
  ...
  const founders = FounderService.getAll() || []; // Safety check agar tidak crash jika data kosong

  const next = () => setCurrentIndex((c) => (c + 1) % founders.length);
  const prev = () => setCurrentIndex((c) => (c - 1 + founders.length) % founders.length);

  // Jika data belum siap, jangan render dulu
  ...
  return (
    <Section className="bg-black border-t border-slate-900">
      ...
      <div className="md:hidden relative max-w-sm mx-auto px-4">
         ...
         {/* Navigation Buttons */}
         <button onClick={prev} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1 p-3 bg-slate-800 rounded-full text-white shadow-lg border border-slate-700 hover:bg-blue-600 transition-colors z-10">
           <ChevronLeft size={20} />
         </button>
         <button onClick={next} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1 p-3 bg-slate-800 rounded-full text-white shadow-lg border border-slate-700 hover:bg-blue-600 transition-colors z-10">
